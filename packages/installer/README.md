...
Tiller Hubs. Its public entry points are `GET /deploy`, `POST /deploy` for an
explicit region selection, `GET /maintenance?intent=update|renew`, and
`GET /stable`.

The installer:

1. infers or asks for the installation region, creates a browser-bound OAuth
   state and PKCE verifier, and redirects directly to Cloudflare;
...
never records it as owned, creates it, changes it, or deletes it. Every v1
release image must be digest-pinned under exactly `docker.io`.
