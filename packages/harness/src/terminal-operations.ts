...
type ControlOperation =
  ...
  | { type: "protocol-reply"; data: string }
  | {
      type: "input";
...
export class TerminalOperationQueue {
  ...
  enqueueOutput(output: string): void {
    ...
    if (barriers.length > 0) {
      let barrierIndex = 0;
      while (this.isProtocolBarrier(this.controls[barrierIndex])) barrierIndex += 1;
  ...
  /** Queue a terminal-protocol reply ahead of ordinary input and resize work. */
  enqueueProtocolReply(data: string): void {
    if (this.closed || data.length === 0) return;
    let barrierIndex = 0;
    while (this.isProtocolBarrier(this.controls[barrierIndex])) barrierIndex += 1;
    this.controls.splice(barrierIndex, 0, { type: "protocol-reply", data });
    this.reportQueueUsage();
    void this.drainControls();
  }
  ...
  private async drainControls(): Promise<void> {
    ...
    try {
      while (this.controls.length > 0) {
        ...
        if (operation.type === "protocol-reply") {
          this.applyProtocolReply(operation);
          continue;
        }
        ...
        try {
          ...
          for (const fragment of operation.fragments) {
            ...
            if (this.isProtocolBarrier(this.controls[0])) {
              await this.drainLeadingProtocolBarriers();
            }
            if (operation.abortGeneration !== this.abortGeneration) throw abortedError();
  ...
  private applyProtocolReply(
    operation: Extract<ControlOperation, { type: "protocol-reply" }>,
  ): void {
    try {
      this.target.write(operation.data);
    } catch (error) {
      this.enterProtocolFault(error);
    }
  }

  private isProtocolBarrier(
    operation: ControlOperation | undefined,
  ): operation is Extract<ControlOperation, { type: "dsr-barrier" | "protocol-reply" }> {
    return operation?.type === "dsr-barrier" || operation?.type === "protocol-reply";
  }

  private async drainLeadingProtocolBarriers(): Promise<void> {
    while (this.isProtocolBarrier(this.controls[0])) {
      const barrier = this.controls.shift();
      if (!barrier) return;
      this.reportQueueUsage();
      if (barrier.type === "dsr-barrier") {
        await this.applyDsrBarrier(barrier);
      } else if (barrier.type === "protocol-reply") {
        this.applyProtocolReply(barrier);
      } else {
        return;
      }
      if (this.protocolFault) throw this.protocolFault;
  ...
  private rejectControl(
    operation: Exclude<ControlOperation, { type: "dsr-barrier" | "protocol-reply" }>,
    error: Error,
  ): void {
