...
const OPENCODE_EXPORT_MAX_CHARS = 16 * 1024 * 1024;
const OPENCODE_EXPORT_DEADLINE_MS = 30_000;
...
async function executeTurn(
  ...
  try {
    ...
    const prompt = buildPrompt(
      context,
      subscriptionAppServer || settings.harness === "opencode"
        ? undefined
        : settings.outputFile,
    );
    ...
    const runProviderAttempt = async (attemptCommand: ProviderCommand) => {
      let stdoutBuffer = "";
      let stderrTail = "";
      const outcome = await superviseDirectChild({
        spawnChild: () => spawn(attemptCommand.command, attemptCommand.args, {
          cwd: settings.checkoutDir,
          detached: true,
      ...
      if (stdoutBuffer.trim()) tracker.handleLine(stdoutBuffer);
      if (tracker.providerSessionId) {
        console.error(`[tiller-planner] provider session id=${tracker.providerSessionId}`);
      }
      await client.flushModelActivity();
      return { command: attemptCommand, outcome, stderrTail };
    };

    const replayOpenCodeSession = async (
      sessionId: string,
      sourceCommand: ProviderCommand,
    ): Promise<{ cancelled: boolean; diagnostic?: string }> => {
      let stdout = "";
      let stdoutChars = 0;
      let stdoutTruncated = false;
      let stderrTail = "";
      const outcome = await superviseDirectChild({
        spawnChild: () => spawn(sourceCommand.command, ["export", sessionId], {
          cwd: settings.checkoutDir,
          detached: true,
          stdio: ["ignore", "pipe", "pipe"],
          env: buildReviewerProviderEnvironment({
            harness: settings.harness,
            source: process.env,
            commandEnv: sourceCommand.env,
            directories: isolation.directories,
          }),
          ...reviewerChildIdentity(isolation.account),
        }),
        isCancelled: async () => await client.pollRunStatus() === "cancelled",
        deadlineMs: OPENCODE_EXPORT_DEADLINE_MS,
        statusPollMs: settings.statusPollMs,
        onSpawn: (child) => {
          child.stdout?.setEncoding("utf-8");
          child.stdout?.on("data", (chunk: string) => {
            stdoutChars += chunk.length;
            if (stdout.length < OPENCODE_EXPORT_MAX_CHARS) {
              stdout += chunk.slice(0, OPENCODE_EXPORT_MAX_CHARS - stdout.length);
            }
            if (stdoutChars > OPENCODE_EXPORT_MAX_CHARS) stdoutTruncated = true;
          });
          child.stderr?.setEncoding("utf-8");
          child.stderr?.on("data", (chunk: string) => {
            stderrTail = `${stderrTail}${chunk}`.slice(-STDERR_TAIL_CHARS);
      ...
      if (outcome.kind === "timed_out") {
        return { cancelled: false, diagnostic: "OpenCode session export timed out after 30 seconds." };
      }
      if (outcome.kind === "spawn_failed") {
        return { cancelled: false, diagnostic: `Failed to export OpenCode session: ${outcome.error.message}` };
      }
      if (outcome.exitCode !== 0) {
        return {
          cancelled: false,
          diagnostic: `OpenCode session export exited with code ${outcome.exitCode ?? "unknown"}${stderrTail.trim() ? `: ${stderrTail.trim()}` : ""}`,
        };
      }
      if (stdoutTruncated) {
        return { cancelled: false, diagnostic: "OpenCode session export exceeded the 16 MiB recovery limit." };
      }
      try {
        tracker.handleOpenCodeSessionExport(stdout);
        await client.flushModelActivity();
        console.error(`[tiller-planner] replayed OpenCode session export id=${sessionId}`);
    ...
    const terminalAttemptOutcome = async (
      attempt: Awaited<ReturnType<typeof runProviderAttempt>>,
    ): Promise<"failed" | "cancelled" | null> => {
      const { outcome, stderrTail } = attempt;
      if (outcome.kind === "cancelled") {
        // The hub already marked the run cancelled; a result would be ignored.
        return "cancelled";
      }
      if (outcome.kind === "timed_out") {
        await failTurn(new Error(`${attempt.command.command} timed out after 60 minutes.`));
        return "failed";
      }
      if (outcome.kind === "spawn_failed") {
        await failTurn(new Error(`Failed to start ${attempt.command.command}: ${outcome.error.message}`));
        return "failed";
      }
      if (outcome.exitCode !== 0) {
        await failTurn(new Error(
          `${attempt.command.command} exited with code ${outcome.exitCode ?? "unknown"}${stderrTail.trim() ? `: ${stderrTail.trim()}` : ""}`,
        ));
        return "failed";
      }
      return null;
    ...
    const readTrackedOutput = () => readOptionalFile(settings.outputFile)
      || readOptionalFile(fallbackOutputFile)
      || (tracker.finalFallbackText ?? "").trim();
    const providerDiagnostics: string[] = [];
    const addProviderDiagnostic = (value: string | null | undefined) => {
      const diagnostic = value?.trim();
      if (diagnostic && !providerDiagnostics.includes(diagnostic)) {
        providerDiagnostics.push(diagnostic);
      }
    };
    const collectAttemptDiagnostics = (providerAttempt: Awaited<ReturnType<typeof runProviderAttempt>>) => {
      addProviderDiagnostic(tracker.providerError);
      addProviderDiagnostic(providerAttempt.stderrTail);
    };
    const recoverExportedSession = async (
      providerAttempt: Awaited<ReturnType<typeof runProviderAttempt>>,
    ): Promise<"cancelled" | null> => {
      const sessionId = tracker.providerSessionId;
      if (!sessionId) return null;
      const recovery = await replayOpenCodeSession(sessionId, providerAttempt.command);
      addProviderDiagnostic(recovery.diagnostic);
      addProviderDiagnostic(tracker.providerError);
      return recovery.cancelled ? "cancelled" : null;
    };
    let attempt = await runProviderAttempt(command);
    let terminalOutcome = await terminalAttemptOutcome(attempt);
    if (terminalOutcome) return terminalOutcome;
    let output = readTrackedOutput();

    if (!output && settings.harness === "opencode") {
      collectAttemptDiagnostics(attempt);
      if (fingerprintReviewerCheckout(settings.checkoutDir) !== checkoutFingerprint) {
        return await failTurn(new Error("Reviewer modified the protected workspace checkout."));
      }
      if (tracker.providerSessionId) {
        console.error("[tiller-planner] OpenCode returned no streamed final response; checking its session export");
        if (await recoverExportedSession(attempt) === "cancelled") return "cancelled";
        output = readTrackedOutput();
      }

      if (!output) {
        const retrySessionId = tracker.providerSessionId;
        console.error(
          retrySessionId
            ? `[tiller-planner] OpenCode session ${retrySessionId} had no final response; continuing it once`
            : "[tiller-planner] OpenCode returned no final response or session id; retrying once",
        );
        tracker.providerError = null;
        client.queueModelActivity("Retrying final response");
        const retryPrompt = retrySessionId
          ? "The previous turn ended without a readable final response. Complete the pending reviewer task and return a non-empty final answer now."
          : [
              prompt,
              "",
              "## Retry delivery requirement",
              "The prior attempt returned no usable final response. Complete the task now and end with a non-empty final answer.",
            ].join("\n");
        const retryCommand = resolveProviderCommand(
          settings,
          context,
          retryPrompt,
          fallbackOutputFile,
        ...
        attempt = await runProviderAttempt(retryCommand);
        terminalOutcome = await terminalAttemptOutcome(attempt);
        if (terminalOutcome) return terminalOutcome;
        output = readTrackedOutput();
        collectAttemptDiagnostics(attempt);
        if (!output && tracker.providerSessionId) {
          console.error("[tiller-planner] OpenCode retry had no streamed final response; checking its session export");
          if (await recoverExportedSession(attempt) === "cancelled") return "cancelled";
          output = readTrackedOutput();
        }
      }
    ...
    if (!output) {
      if (settings.harness === "opencode") {
        const diagnostic = providerDiagnostics.filter(Boolean).join(" | ").slice(0, STDERR_TAIL_CHARS);
        return await failTurn(new Error(
          `opencode completed without returning a final response${diagnostic ? `: ${diagnostic}` : ""}`,
        ));
      }
