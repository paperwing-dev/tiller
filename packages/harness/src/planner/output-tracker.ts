...
function opencodeProviderError(parsed: Record<string, unknown>): string {
  if (parsed.type !== "error") return "";
  if (typeof parsed.error === "string") return parsed.error.trim().slice(0, 2_000);
  if (!isRecord(parsed.error)) return "OpenCode reported an unknown provider error.";
  const data = isRecord(parsed.error.data) ? parsed.error.data : null;
  const message = typeof data?.message === "string"
    ? data.message
    : typeof parsed.error.message === "string"
      ? parsed.error.message
      : "";
  const name = typeof parsed.error.name === "string" ? parsed.error.name.trim() : "";
  return [name, message.trim()]
    .filter(Boolean)
    .join(": ")
    .slice(0, 2_000) || "OpenCode reported an unknown provider error.";
}
...
function openCodeSessionExportLines(serialized: string): string[] {
  let exported: unknown;
  try {
    exported = JSON.parse(serialized);
  } catch {
    throw new Error("OpenCode session export was not valid JSON.");
  }
  if (!isRecord(exported) || !Array.isArray(exported.messages)) {
    throw new Error("OpenCode session export did not contain messages.");
  }
  ...
  for (const message of exported.messages) {
    ...
    const messageSessionId = opencodeSessionId(info);
    ...
    for (const part of message.parts) {
      ...
      const type = part.type === "tool"
        ? "tool_use"
        : part.type === "text"
          ? "text"
          : null;
      if (!type) continue;
      const sessionId = opencodeSessionId(part) || messageSessionId;
      lines.push(JSON.stringify({
        type,
...
export function parsePlannerLine(harness: Harness, line: string): ParsedPlannerLine | null {
  ...
  if (harness === "opencode") {
    ...
    const providerError = opencodeProviderError(parsed);
    if (assistantText || activity || providerError) {
      return {
...
export class PlannerOutputTracker {
  ...
  handleLine(line: string): void {
    ...
    if (parsed.providerError) this.providerError = parsed.providerError;
  ...
  handleOpenCodeSessionExport(serialized: string): void {
    if (this.harness !== "opencode") {
      throw new Error("Only OpenCode output can be replayed from an OpenCode session export.");
    }
    for (const line of openCodeSessionExportLines(serialized)) {
