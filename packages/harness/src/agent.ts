...
import {
  ...
  type TerminalPaletteFilterEvent,
} from "./vt-palette-query-filter.js";
...
export class Agent extends EventEmitter<AgentEvents> {
  ...
  private readonly paletteQueryFilter: TerminalPaletteQueryFilter | null;
  constructor(
    command: string,
    args: string[],
    cwd: string,
    ...
    this.paletteQueryFilter = options.terminalPalette
      ? new TerminalPaletteQueryFilter(options.terminalPalette)
      : null;
    ...
    this.ptyProcess.onExit(({ exitCode }) => {
      ...
      for (const event of this.paletteQueryFilter?.end() ?? []) {
        this.handlePaletteFilterEvent(event);
      }
  ...
  private handlePtyOutput(data: string): void {
    if (!this.paletteQueryFilter) {
      this.terminalOperations.enqueueOutput(data);
      return;
    }
    for (const event of this.paletteQueryFilter.push(data)) {
      this.handlePaletteFilterEvent(event);
    }
  }

  private handlePaletteFilterEvent(event: TerminalPaletteFilterEvent): void {
    if (event.type === "output") {
      this.terminalOperations.enqueueOutput(event.data);
    } else {
      this.terminalOperations.enqueueProtocolReply(event.data);
    }
  }
