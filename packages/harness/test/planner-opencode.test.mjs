...
import { buildArgs, buildOpenCodeProxyConfig } from "../dist/planner/providers/opencode.js";
...
test("buildArgs uses the provider/model alias and writes the config", () => {
  ...
  try {
    ...
    // `-m` must be provider/model format — never the raw model id.
    assert.deepEqual(command.args.slice(0, 5), ["run", "--format", "json", "--model", "tiller-hub/tiller-kimi-k2-7-code"]);
...
test("buildArgs omits the model flag for raw ids without proxy config", () => {
  ...
  assert.deepEqual(command.args, ["run", "--format", "json", "Plan."]);
});
