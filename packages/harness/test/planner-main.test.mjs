...
const FAKE_OPENCODE_FINAL = `#!/usr/bin/env node
const prompt = process.argv.at(-1) || "";
if (!prompt.includes("Return your complete response in your final answer.") || prompt.includes("output/review.md")) {
  console.error("OpenCode reviewer received a file-output prompt");
  process.exit(4);
}
const config = JSON.parse(process.env.OPENCODE_CONFIG_CONTENT || "{}");
const headers = config.provider?.["tiller-hub"]?.options?.headers;
if (headers?.["CF-Access-Client-Id"] !== "access-id" || headers?.["CF-Access-Client-Secret"] !== "access-secret") {
  console.error("OpenCode reviewer lost the Hub Access transport headers");
  process.exit(4);
}
console.log(JSON.stringify({ type: "session.created", session: { id: "oc-session-e2e" } }));
...
console.log(JSON.stringify({
  type: "text",
  part: { type: "text", text: "Final streamed OpenCode review." },
}));
`;

const FAKE_OPENCODE_RETRY = `#!/usr/bin/env node
const { existsSync, writeFileSync } = require("node:fs");
const marker = process.env.TILLER_PLANNER_OUTPUT_FILE + ".attempted";
const args = process.argv.slice(2);
if (args[0] === "export") {
  console.log(JSON.stringify({
    info: { id: "oc-session-retry" },
    messages: [
      { info: { role: "user", sessionID: "oc-session-retry" }, parts: [{ type: "text", text: "Original prompt" }] },
      { info: { role: "assistant", sessionID: "oc-session-retry" }, parts: [] },
    ],
  }));
  process.exit(0);
}
const prompt = process.argv.at(-1) || "";
console.log(JSON.stringify({ type: "session.created", session: { id: "oc-session-retry" } }));
if (!existsSync(marker)) {
  writeFileSync(marker, "1");
  console.log(JSON.stringify({
    type: "error",
    sessionID: "oc-session-retry",
    error: { name: "UnknownError", data: { message: "Type validation failed" } },
  }));
  process.exit(0);
}
const sessionIndex = args.indexOf("--session");
if (args[sessionIndex + 1] !== "oc-session-retry") {
  console.error("OpenCode retry did not continue the original session");
  process.exit(5);
}
if (!prompt.includes("previous turn ended without a readable final response")) {
  console.error("OpenCode continuation prompt was not strengthened");
  process.exit(5);
}
console.log(JSON.stringify({
  type: "tool_use",
...
console.log(JSON.stringify({
  type: "text",
  part: { type: "text", text: "Recovered OpenCode review." },
}));
`;

const FAKE_OPENCODE_EXPORT_RECOVERY = `#!/usr/bin/env node
const args = process.argv.slice(2);
if (args[0] === "export") {
  console.log(JSON.stringify({
    info: { id: "oc-session-export" },
    messages: [
      { info: { role: "user", sessionID: "oc-session-export" }, parts: [{ type: "text", text: "Do not return user text." }] },
      {
        info: { role: "assistant", sessionID: "oc-session-export" },
        parts: [
          {
            type: "tool",
            tool: "read",
            sessionID: "oc-session-export",
            state: { status: "completed", input: { filePath: "README.md" } },
          },
          { type: "text", sessionID: "oc-session-export", text: "Recovered from the OpenCode session export." },
        ],
      },
    ],
  }));
...
if (!args.includes("--title") || !args.includes("Tiller reviewer")) {
  console.error("OpenCode reviewer did not receive an explicit title");
  process.exit(6);
}
console.log(JSON.stringify({ type: "step_start", sessionID: "oc-session-export", part: { type: "step-start" } }));
`;
...
function writeFakeCli(root, script, cliName = "claude") {
...
test("OpenCode reviewer returns streamed final text without writing review.md", async () => {
  const root = mkdtempSync(join(tmpdir(), "planner-opencode-e2e-"));
  const { repoUrl, commit } = createRepo(root);
  const binDir = writeFakeCli(root, FAKE_OPENCODE_FINAL, "opencode");
  const context = buildContext(commit);
  context.run.provider = "opencode";
  context.run.model = "@cf/moonshotai/kimi-k2.7-code";
  context.input.effort = "high";
  const { server, received, port } = await startHubStub({
    context,
    runStatusFn: () => "running",
  });
  try {
    const { exited, stderrText } = runPlannerMain({
      root,
      port,
      binDir,
      repoUrl,
      ...
      extraEnv: {
        ...
        TILLER_OPENCODE_BASE_URL: "https://hub.invalid/api/opencode/v1",
        TILLER_OPENCODE_AUTH_TOKEN: "test-token",
    ...
    assert.deepEqual(received.results, [
      { status: "succeeded", text: "Final streamed OpenCode review." },
    ]);
    assert.equal(received.events.some((event) => event.message === "read: README.md"), true);
  } finally {
    server.close();
    cleanupRoot(root);
  }
...
test("OpenCode reviewer recovers a missing stream response from its session export", async () => {
  const root = mkdtempSync(join(tmpdir(), "planner-opencode-export-e2e-"));
  const { repoUrl, commit } = createRepo(root);
  const binDir = writeFakeCli(root, FAKE_OPENCODE_EXPORT_RECOVERY, "opencode");
  const context = buildContext(commit);
  context.run.provider = "opencode";
  context.run.model = "@cf/moonshotai/kimi-k2.7-code";
  context.input.effort = "high";
  ...
  try {
    ...
    assert.deepEqual(received.results, [{
      status: "succeeded",
      text: "Recovered from the OpenCode session export.",
    }]);
    assert.equal(received.events.some((event) => event.message === "read: README.md"), true);
    assert.equal(
      received.events.some((event) => event.message === "Retrying final response"),
      false,
    );
    assert.match(stderrText(), /replayed OpenCode session export id=oc-session-export/);
  } finally {
    server.close();
    cleanupRoot(root);
  }
...
test("OpenCode reviewer retries a zero-exit provider error and returns the second final response", async () => {
  const root = mkdtempSync(join(tmpdir(), "planner-opencode-retry-e2e-"));
  const { repoUrl, commit } = createRepo(root);
  const binDir = writeFakeCli(root, FAKE_OPENCODE_RETRY, "opencode");
  const context = buildContext(commit);
  context.run.provider = "opencode";
  context.run.model = "@cf/moonshotai/kimi-k2.7-code";
  context.input.effort = "high";
  ...
  try {
    ...
    assert.equal(await exited, 0, stderrText());
    assert.deepEqual(received.results, [
      { status: "succeeded", text: "Recovered OpenCode review." },
    ]);
    assert.equal(
      received.events.some((event) => event.message === "Retrying final response"),
      true,
    );
    assert.match(stderrText(), /OpenCode session oc-session-retry had no final response; continuing it once/);
  } finally {
    server.close();
    cleanupRoot(root);
  }
