...
test("a reentrant protocol reply stays outside submitted input fragments", async () => {
  const headless = new FakeHeadless();
  const events = [];
  let queue;
  queue = new TerminalOperationQueue(
    {
      write: (data) => {
        events.push(["pty-write", data]);
        if (data === "hello") queue.enqueueProtocolReply("palette-reply");
      },
      resize: () => {},
    },
    { cols: 80, rows: 24 },
  ...
  await queue.enqueueInput([
    { data: "hello", delayMs: 0 },
    { data: "\r", delayMs: 1 },
  ]);
  assert.deepEqual(events, [
    ["pty-write", "hello"],
    ["pty-write", "palette-reply"],
    ["pty-write", "\r"],
  ]);
});
