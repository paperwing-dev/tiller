...
import { buildArgs as buildOpenCodeReviewerArgs } from "../dist/planner/providers/opencode.js";
import { PlannerOutputTracker } from "../dist/planner/output-tracker.js";
import {
...
function reviewerOpenAIResponse(response, input) {
  response.writeHead(200, {
    "Content-Type": "text/event-stream",
    "Cache-Control": "no-cache",
    Connection: "keep-alive",
  ...
  const chunk = (value) => response.write(`data: ${JSON.stringify(value)}\n\n`);
  const base = {
    id: `reviewer-response-${input.requestIndex}`,
    object: "chat.completion.chunk",
    created: 1,
    model: input.modelId,
  };
      ...
      choices: [
        {
          ...
          delta: {
            role: "assistant",
            content: input.main
              ? "OpenCode reviewer output."
              : "Reviewer smoke title",
          },
          finish_reason: null,
        },
      ],
...
test("OpenCode image returns a one-shot reviewer final answer", async () => {
  const root = await mkdtemp(join(tmpdir(), "tiller-image-reviewer-"));
  const checkout = join(root, "checkout");
  await mkdir(checkout);
  const factPath = join(checkout, "facts.txt");
  await writeFile(factPath, "Repository reviewer fact.\n");
  const { account, directories } = prepareReviewerRuntimeDirectories(
    checkout,
    "image-reviewer-smoke",
  );
  ...
  const providerServer = createServer(async (request, response) => {
    ...
    const body = JSON.parse(raw);
    const main = Array.isArray(body.tools) && body.tools.length > 0;
    if (main) mainRequestCount += 1;
    reviewerOpenAIResponse(response, {
      main,
      requestIndex: main ? mainRequestCount : 0,
      factPath,
      modelId: cases[2].modelId,
    });
  });
  await listen(providerServer, 0, "127.0.0.1");
  const address = providerServer.address();
  ...
  let child;
  try {
    const source = selectionEnv({
      ...cases[2],
      baseURL: `http://127.0.0.1:${address.port}/v1`,
    });
    const providerEnv = buildReviewerProviderEnvironment({
      harness: "opencode",
      source,
      directories,
    });
    const command = buildOpenCodeReviewerArgs({
      prompt: "Read facts.txt, then return the complete review in your final answer.",
      model: cases[2].modelId,
      effort: "high",
      env: providerEnv,
      configPath: join(directories.config, "opencode.json"),
    });
    ...
    const exitCode = await Promise.race([
      ...
      delay(60_000).then(() => {
        child.kill("SIGKILL");
        throw new Error(
          `OpenCode reviewer smoke timed out after ${mainRequestCount} requests. stdout=${stdout} stderr=${stderr}`,
        );
      }),
    ]);
    child = undefined;
    ...
    assert.doesNotMatch(stderr, /permission requested|auto-rejecting/i);
    const lines = stdout
      .trim()
      .split("\n")
      .filter(Boolean);
    const tracker = new PlannerOutputTracker("opencode", undefined, checkout);
    for (const line of lines) tracker.handleLine(line);
    assert.equal(tracker.hasSuccessfulRepositoryInspection, true);
    assert.equal(tracker.finalFallbackText, "OpenCode reviewer output.");
    await assert.rejects(readFile(directories.output, "utf8"), /ENOENT/);
  } finally {
    child?.kill("SIGKILL");
