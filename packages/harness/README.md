...
- `CF_ACCESS_CLIENT_ID` and `CF_ACCESS_CLIENT_SECRET`, or the corresponding
  config fields, for a protected workers.dev Hub
...
```bash
TILLER_HARNESS=claude-code HUB_URL=https://example.workers.dev tiller-harness
...
- `--bare` uses Claude API-key auth instead of OAuth/keychain auth while
  retaining the permission hook.
- `--skip-permissions` removes Claude's permission hook and runs it
  autonomously. Codex already bypasses approvals and its sandbox; OpenCode
  ignores both flags.
...
The package includes `dist/`, `hooks/`, and the OpenCode image smoke test.
