...
Release automation requires the public `paperwing-dev/tiller` repository, a
GitHub App with Contents and Actions write access, a public push credential that
may update workflows, and the Docker Hub secrets used by
`container-image.yml`. The manual `reset_public_history` option starts `main`
at a new root commit; it does not delete existing tags, releases, or Actions
logs. Keep the former `paperwing-dev/tiller-hub` repository publicly readable
so descriptors from older installations can still fetch their release assets.
