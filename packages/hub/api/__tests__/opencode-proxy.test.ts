...
describe("OpenCode proxy routes", () => {
  ...
  it("filters Kimi's trailing binding summary from OpenAI-compatible streams", async () => {
    const encoder = new TextEncoder();
    const stream = new ReadableStream({
      start(controller) {
        for (const event of [
          ...
          { choices: [{ index: 0, delta: { content: "review complete" }, finish_reason: null }] },
          { choices: [{ index: 0, delta: {}, finish_reason: "stop" }] },
          { choices: [], usage: { prompt_tokens: 0, completion_tokens: 1, total_tokens: 1 } },
          { response: "", usage: { prompt_tokens: 24, completion_tokens: 44, total_tokens: 68 } },
        ]) {
    ...
    const res = await app.request(
      ...
      {
        [OPENCODE_PROXY_TOKEN_KEY]: "proxy-token-123",
        AI: { run: vi.fn().mockResolvedValue(stream) },
      } as any,
    );

    const body = await res.text();
    expect(body).toContain('"delta":{"content":"review complete"}');
    expect(body).toContain('"finish_reason":"stop"');
