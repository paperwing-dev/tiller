...
async function close(server) {
  await new Promise((resolve, reject) => {
    ...
    // The Codex child can leave its final HTTP connection alive briefly after
    // the assertions complete. Stop accepting connections first, then force
    // any remaining smoke-only connections closed so CI teardown is bounded.
